...
import psutil
import config
from Demonchat import _boot_
from Demonchat import get_readable_time
from Demonchat import Demonchat, mongo, SUDOERS
...
from Demonchat.database.chats import get_served_chats, add_served_chat
from Demonchat.database.users import get_served_users, add_served_user
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, Message, CallbackQuery
from Demonchat.modules.helpers import (
    START,
    START_BOT,
    PNG_BTN,
    CLOSE_BTN,
...
@Demonchat.on_message(filters.new_chat_members)
        ...
        for member in message.new_chat_members:
            ...
            if member.id == Demonchat.id:
                try:
                    ...
                    pass
                try:
                    invitelink = await Demonchat.export_chat_invite_link(message.chat.id)
                ...
                try:
                    groups_photo = await Demonchat.download_media(
                ...
                count = await Demonchat.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
...
@Demonchat.on_cmd(["start", "aistart"])
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await Demonchat.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(Demonchat.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        ...
        await Demonchat.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
...
@Demonchat.on_cmd("help")
async def help(client: Demonchat, m: Message):
    if m.chat.type == ChatType.PRIVATE:
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{Demonchat.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve 🥀 αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||**๏ мαdє ωιтн ❣️ ву [ʟᴇɢᴇɴᴅ ᴍɪᴄᴋᴇʏ](https://t.me/{OWNER_USERNAME}) **||</b>",
...
@Demonchat.on_message(filters.command("stats"))
...
from Demonchat import Demonchat
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await Demonchat.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await Demonchat.send_message(
