...
from Demonchat import CLONE_OWNERS
from Demonchat import Demonchat as app, save_clonebot_owner
from Demonchat import db as mongodb, Demonchat
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="Demonchat/mplugin"))
            await ai.start()
            bot = await ai.get_me()
            bot_id = bot.id
